      ...
      <div className={cn(
        ...
        <form id="category-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 space-y-6">
          ...
          <div>
            ...
            {coverImage ? (
              <div className="relative">
                <div className="w-full h-48 bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                  <img 
                    src={coverImage.startsWith('http') ? coverImage : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}${coverImage.startsWith('/') ? '' : '/'}${coverImage}`}
                    alt={t('categories.coverPreview')}
                    className="w-full h-full object-cover"
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      target.style.display = 'none';
                      target.parentElement!.innerHTML = '<div class="w-full h-full flex items-center justify-center text-gray-400"><ImageIcon size={48} /></div>';
                    }}
                  />
                </div>
                <button
              ...
              >
                ...
                <p className="text-sm text-gray-600">{t('categories.clickToUploadCoverImage')}</p>
            ...
            />
            {isUploadingImage && (
              <p className="text-xs text-gray-500 mt-1">{t('categories.uploadingImage')}</p>
            )}
          </div>
